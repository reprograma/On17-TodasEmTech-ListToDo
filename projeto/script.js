...
const botaoMarcaTodas = document.getElementById('botao_marca_id');

const listaDeItens = document.getElementsByClassName('texto');
...
botaoAdd.addEventListener('click', (evento) => {
    ...
    novaTarefaInput.setAttribute("style",'border-color:#c8d6e5;')
    
    if (novaTarefaInput.value.trim() === '') {
        alert("digite alguma tarefa")
        novaTarefaInput.setAttribute("style",'border-color:red;')
    } else {
...
// mudando botao de marcar e desmarcar
botaoMarcaTodas.addEventListener('click', () => {
    const textoArray = Array.from(listaDeItens)
    if (botaoMarcaTodas.innerText == 'Marcar todas') {
        for (let i of textoArray) {
            i.classList.remove('checked')
        }
        botaoMarcaTodas.innerText = 'Desmarcar todas'
    } else {
        for (let i of textoArray) {
            i.classList.add('checked')
        }
        botaoMarcaTodas.innerText = 'Marcar todas'
    }
})
